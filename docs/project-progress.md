...
- 真实数据把卡片身份问题暴露出来了：独角鲸云那两台的主机名就是厂商给的容器 UUID，而副行原来只在 `sample.hostname !== facts.hostname` 时才渲染，于是三张卡除了一个被截断的 UUID 之外毫无区别。现在副行固定给 `provider / region`（与运维终端、节点详情同一文案），标题行带 `title` 承载完整主机名。

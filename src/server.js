...
import { createBootstrapTokenValidators } from "./http/validators.js";
import {
  getPreferredLanIpv4,
  isLoopbackHost,
  isPublicIpv4,
...
const dataDir = path.resolve(process.env.AIRPORT_DATA_DIR ?? path.resolve(currentDir, "../data"));
...
const server = createServer(createSafeRequestHandler(async (request, reply) => {
  ...
  for (const handleApiRoutes of apiRoutes) {
    await handleApiRoutes({ request, reply, url });
    if (reply.headersSent || reply.writableEnded) {
      return;
    }
  }
...
const { start } = createServerStartupRuntime({
  ...
  listen: () => {
    server.listen(port, () => {
      ...
      console.log(`airport-control-plane listening on http://localhost:${listeningPort}`);
    });
  },
  startProbeScheduler,
